...
class Account implements IModel
{
    ...
    public function __construct($pdo, $data = null)
    {
        ...
        $this->_modelName = 'Account';
        $this->_fieldsOnly = ['id', 'userId', 'type', 'provider', 'providerAccountId', 'refresh_token', 'access_token', 'expires_at', 'token_type', 'scope', 'id_token', 'session_state'];
        ...
        $this->_col = new class()
        {
            public function __construct(
                ...
                public readonly string $providerAccountId = 'providerAccountId',
                public readonly string $refresh_token = 'refresh_token',
                public readonly string $access_token = 'access_token',
                public readonly string $expires_at = 'expires_at',
                public readonly string $token_type = 'token_type',
                public readonly string $scope = 'scope',
                public readonly string $id_token = 'id_token',
                public readonly string $session_state = 'session_state',
        ...
        if ($data) {
            ...
            $this->providerAccountId = $data['providerAccountId'] ?? null;
            ...
            $this->session_state = $data['session_state'] ?? null;
    ...
    {
        $validatedValue = Validator::validateString($value);
        $this->providerAccountId = $validatedValue;
    }
    ...
    {
        $validatedValue = Validator::validateString($value);
        $this->refresh_token = $validatedValue;
    }
    ...
    {
        $validatedValue = Validator::validateString($value);
        $this->access_token = $validatedValue;
    }
    ...
    public function setExpires_at($value)
    {
        ...
        $this->expires_at = $validatedValue;
    }
    ...
    {
        $validatedValue = Validator::validateString($value);
        $this->token_type = $validatedValue;
    }
    ...
    public function setId_token($value)
    {
        $validatedValue = Validator::validateString($value);
        $this->id_token = $validatedValue;
    }

    public function getSession_state()
    {
        return $this->session_state;
    }

    public function setSession_state($value)
    {
        $validatedValue = Validator::validateString($value);
        $this->session_state = $validatedValue;
    }

    protected function includeUser(array $items, array $selectedFields = [], array $includeSelectedFields = []) 
    {
        if (empty($items)) {
            return $items;
        }
        ...
        $dbType = $this->_dbType;
        $quotedTableName = $dbType == 'pgsql' ? "\"User\"" : "`User`";
        $tableName = $dbType == 'pgsql' ? "\"Account\"" : "`Account`";
        $tableModelName = 'Account';
        $tablePrimaryKey = 'id';
        ...
        if (!empty($foreignKeyIds)) {
            ...
            foreach ($items as &$item) {
                ...
                $relatedRecords = $tableModelName->findMany(['where' => [$primaryKey => $item[$foreignKey]], 'select' => $includeSelectedFields]);
    ...
    {
        if (!isset($data['data'])) {
            throw new \Exception("The 'data' key is required when creating a new Account.");
        }
        ...
        foreach ($requiredFieldsMap as $fieldName => $fieldType) {
            if (!isset($data['data'][$fieldName])) {
                throw new \Exception("The '$fieldName' field of type '$fieldType' is required when creating a new Account.");
            }
        }
        ...
        $bindings = [];
        $dbType = $this->_dbType;
        $quotedTableName = $dbType == 'pgsql' ? "\"Account\"" : "`Account`";
                ...
                foreach ($data as $relationName => $relationDataName) {
                    ...
                    if (in_array($relationName, $relationNames)) {
                        ...
                        if (isset($relationDataName['create']) || isset($relationDataName['createMany']) || isset($relationDataName['connect']) || isset($relationDataName['connectOrCreate'])) {
                            throw new \Exception("The relation name '$relationName' is not defined in the Account model.");
                        }
                    }
                }
            }
    ...
    public function findUnique(array $criteria, string $format = 'array'): array | object
    {
        ...
        if (isset($where['provider'])) {
            ...
            $validatedValue = Validator::validateString($where['provider']);
            $bindings[':provider'] = $validatedValue;
        }
        if (isset($where['providerAccountId'])) {
            $conditions[] = "providerAccountId = :providerAccountId";
            $validatedValue = Validator::validateString($where['providerAccountId']);
            $bindings[':providerAccountId'] = $validatedValue;
        }
        if (empty($conditions)) {
            throw new \Exception("No valid 'where' conditions provided for finding a unique record in Account.");
        }
        ...
        foreach ($includes as $relation => $include) {
            if ($include) {
                ...
                if (method_exists($this, $includeMethodName)) {
                    ...
                    throw new \Exception("The '$relation' does not exist, in the Account model.");
                }
            }
        }
        ...
        if ($format === 'object') {
            return new Account($this->_pdo, $record);
        }
    ...
    public function findMany(array $criteria = [], string $format = 'array'): array | object
    {
        ...
        if ($format === 'object') {
            $result = [];
            foreach ($items as $item) {
                $result[] = new Account($this->_pdo, $item); // Convert each item into a User object.
            }
            return $result;
        }
    ...
    {
        if (!isset($data['where'])) {
            throw new \Exception("The 'where' key is required in the update Account.");
        }
        ...
        if (!isset($data['data'])) {
            throw new \Exception("The 'data' key is required in the update Account.");
        }
        ...
        foreach ($requiredFieldsMap as $fieldName => $fieldType) {
            if (isset($data['data'][$fieldName]) && empty($data['data'][$fieldName])) {
                throw new \Exception("The '$fieldName' field of type '$fieldType' is required in Account Model.");
            }
        }
                ...
                if (!empty($where)) {
                    ...
                    foreach ($where as $fieldName => $fieldValue) {
                        if (array_key_exists($fieldName, $this->_fields)) {
                            ...
                            throw new \Exception("The '$fieldName' field does not exist in the Account model.");
                        }
                    }
    ...
    public function aggregate(array $operation, string $format = 'array'): array | object
    {
        ...
        $quotedTableName = $this->_dbType == 'pgsql' ? "\"Account\"" : "`Account`";
        $conditions = [];
        $bindings = [];
    ...
    /**
     * Groups records in the 'Account' table and performs aggregate operations.
     * 
    ...
    {
        if (!isset($data['where'])) {
            throw new \Exception("The 'where' key is required in the updateMany Account.");
        }
        ...
        if (!isset($data['data'])) {
            throw new \Exception("The 'data' key is required in the updateMany Account.");
        }
...
";
    ...
    /**
     * Counts the number of records in the 'Account' table based on provided criteria.
     * 
     ...
     * $activeUserCount = $prisma->Account->count($countCriteria);
     */
