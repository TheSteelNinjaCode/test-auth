...
class Session implements IModel
{
    ...
    public function __construct($pdo, $data = null)
    {
        ...
        $this->_modelName = 'Session';
        $this->_fieldsOnly = ['id', 'sessionToken', 'userId', 'expires'];
        ...
        $this->_col = new class()
        {
            public function __construct(
                ...
                public readonly string $sessionToken = 'sessionToken',
    ...
    {
        $validatedValue = Validator::validateString($value);
        $this->sessionToken = $validatedValue;
    }
    ...
    protected function includeUser(array $items, array $selectedFields = [], array $includeSelectedFields = []) 
    {
        ...
        $tableName = $dbType == 'pgsql' ? "\"Session\"" : "`Session`";
        $tableModelName = 'Session';
        $tablePrimaryKey = 'id';
    ...
    {
        if (!isset($data['data'])) {
            throw new \Exception("The 'data' key is required when creating a new Session.");
        }
        ...
        foreach ($requiredFieldsMap as $fieldName => $fieldType) {
            if (!isset($data['data'][$fieldName])) {
                throw new \Exception("The '$fieldName' field of type '$fieldType' is required when creating a new Session.");
            }
        }
        ...
        $bindings = [];
        $dbType = $this->_dbType;
        $quotedTableName = $dbType == 'pgsql' ? "\"Session\"" : "`Session`";
                ...
                foreach ($data as $relationName => $relationDataName) {
                    ...
                    if (in_array($relationName, $relationNames)) {
                        ...
                        if (isset($relationDataName['create']) || isset($relationDataName['createMany']) || isset($relationDataName['connect']) || isset($relationDataName['connectOrCreate'])) {
                            throw new \Exception("The relation name '$relationName' is not defined in the Session model.");
                        }
                    }
                }
            }
    ...
    public function findUnique(array $criteria, string $format = 'array'): array | object
    {
        ...
        if (isset($where['sessionToken'])) {
            $conditions[] = "sessionToken = :sessionToken";
            $validatedValue = Validator::validateString($where['sessionToken']);
            $bindings[':sessionToken'] = $validatedValue;
        }
        if (empty($conditions)) {
            throw new \Exception("No valid 'where' conditions provided for finding a unique record in Session.");
        }
        ...
        foreach ($includes as $relation => $include) {
            if ($include) {
                ...
                if (method_exists($this, $includeMethodName)) {
                    ...
                    throw new \Exception("The '$relation' does not exist, in the Session model.");
                }
            }
        }
        ...
        if ($format === 'object') {
            return new Session($this->_pdo, $record);
        }
    ...
    public function findMany(array $criteria = [], string $format = 'array'): array | object
    {
        ...
        if ($format === 'object') {
            $result = [];
            foreach ($items as $item) {
                $result[] = new Session($this->_pdo, $item); // Convert each item into a User object.
            }
            return $result;
        }
    ...
    {
        if (!isset($data['where'])) {
            throw new \Exception("The 'where' key is required in the update Session.");
        }
        ...
        if (!isset($data['data'])) {
            throw new \Exception("The 'data' key is required in the update Session.");
        }
        ...
        foreach ($requiredFieldsMap as $fieldName => $fieldType) {
            if (isset($data['data'][$fieldName]) && empty($data['data'][$fieldName])) {
                throw new \Exception("The '$fieldName' field of type '$fieldType' is required in Session Model.");
            }
        }
                ...
                if (!empty($where)) {
                    ...
                    foreach ($where as $fieldName => $fieldValue) {
                        if (array_key_exists($fieldName, $this->_fields)) {
                            ...
                            throw new \Exception("The '$fieldName' field does not exist in the Session model.");
                        }
                    }
    ...
    public function aggregate(array $operation, string $format = 'array'): array | object
    {
        ...
        $quotedTableName = $this->_dbType == 'pgsql' ? "\"Session\"" : "`Session`";
        $conditions = [];
        $bindings = [];
    ...
    /**
     * Groups records in the 'Session' table and performs aggregate operations.
     * 
    ...
    {
        if (!isset($data['where'])) {
            throw new \Exception("The 'where' key is required in the updateMany Session.");
        }
        ...
        if (!isset($data['data'])) {
            throw new \Exception("The 'data' key is required in the updateMany Session.");
        }
...
";
    ...
    /**
     * Counts the number of records in the 'Session' table based on provided criteria.
     * 
     ...
     * $activeUserCount = $prisma->Session->count($countCriteria);
     */
