...
class UserRole implements IModel
{
    ...
    public function findUnique(array $criteria, string $format = 'array'): array | object
    {
        ...
        if (isset($where['id'])) {
            $conditions[] = "id = :id";
            $validatedValue = Validator::validateInt($where['id']);
            $bindings[':id'] = $validatedValue;
        }
        if (isset($where['name'])) {
            $conditions[] = "name = :name";
            $validatedValue = Validator::validateString($where['name']);
        ...
        if ($format === 'object') {
            return new UserRole($this->_pdo, $record);
        }
    ...
    public function findMany(array $criteria = [], string $format = 'array'): array | object
    {
        ...
        if ($format === 'object') {
            $result = [];
            foreach ($items as $item) {
                $result[] = new UserRole($this->_pdo, $item); // Convert each item into a User object.
            }
            return $result;
        }
...
";
    ...
    /**
     * Counts the number of records in the 'UserRole' table based on provided criteria.
     * 
     ...
     * $activeUserCount = $prisma->UserRole->count($countCriteria);
     */
