...
class User implements IModel
{
    ...
    public function __construct($pdo, $data = null)
    {
        ...
        $this->_fieldsOnly = ['id', 'name', 'email', 'emailVerified', 'image', 'password', 'userRoleId'];
        $this->_fieldsRelated = ['accounts', 'sessions', 'userRole'];
        ...
        $this->_col = new class()
        {
            public function __construct(
                ...
                public readonly string $userRoleId = 'userRoleId',
            ) {
            }
        };
        ...
        if ($data) {
            ...
            $this->accounts = new Account($this->_pdo, $data['accounts'] ?? null);
            $this->sessions = new Session($this->_pdo, $data['sessions'] ?? null);
    ...
    public function setUserRoleId($value)
    {
        ...
        $this->userRoleId = $validatedValue;
    }
    ...
    protected function includeUserRole(array $items, array $selectedFields = [], array $includeSelectedFields = []) 
    {
        ...
        $foreignKeyIds = array_column($items, 'userRoleId');
        $primaryKey = 'id';
        $foreignKey = 'userRoleId';
        $primaryKeyQuoted = $dbType == 'pgsql' ? "\"id\"" : "`id`";
        $foreignKeyQuoted = $dbType == 'pgsql' ? "\"userRoleId\"" : "`userRoleId`";
    ...
    protected function includeAccounts(array $items, array $selectedFields = [], array $includeSelectedFields = []) 
    {
        if (empty($items)) {
            return $items;
        }
        ...
        $modelName = new Account($this->_pdo);
        foreach ($items as &$item) {
            if (!isset($item[$primaryKey])) {
                $item[$relationName] = [];
                continue;
            }
            $relatedRecords = $modelName->findMany(['where' => [$foreignKey => $item[$primaryKey]], 'select' => $includeSelectedFields]);
            $item[$relationName] = $relatedRecords;
        }
    ...
    protected function connectAccounts(string $relationName, array $connectData, string $lastInsertId, string $connectType = 'connect')
    {
        $dbType = $this->_dbType;
        $quotedTableName = $dbType == 'pgsql' ? "\"User\"" : "`User`";
        $tableName = $dbType == 'pgsql' ? "\"user\"" : "`user`";
        ...
        $relationModel = new Account($this->_pdo);
        ...
        if (!empty($foundUnique) && $connectType === 'connect') {
            try {
                ...
                throw new \Exception("Error connecting accounts: " . $e->getMessage());
            }
        }
        ...
        if (!empty($foundUnique) && $connectType === 'disconnect') {
            try {
                ...
                throw new \Exception("Error disconnecting accounts: " . $e->getMessage());
            }
        }
    }
    protected function includeSessions(array $items, array $selectedFields = [], array $includeSelectedFields = []) 
    {
        if (empty($items)) {
            return $items;
        }
        ...
        $modelName = new Session($this->_pdo);
        foreach ($items as &$item) {
            if (!isset($item[$primaryKey])) {
                $item[$relationName] = [];
                continue;
    ...
    protected function connectSessions(string $relationName, array $connectData, string $lastInsertId, string $connectType = 'connect')
    {
        $dbType = $this->_dbType;
        $quotedTableName = $dbType == 'pgsql' ? "\"User\"" : "`User`";
        $tableName = $dbType == 'pgsql' ? "\"user\"" : "`user`";
        ...
        $relationModel = new Session($this->_pdo);
        ...
        if (!empty($foundUnique) && $connectType === 'connect') {
            try {
                ...
                throw new \Exception("Error connecting sessions: " . $e->getMessage());
            }
        }
        ...
        if (!empty($foundUnique) && $connectType === 'disconnect') {
            try {
                ...
                throw new \Exception("Error disconnecting sessions: " . $e->getMessage());
            }
        }
    }
    ...
    public function create(array $data, string $format = 'array'): array | object 
    {
        ...
        $data = $data['data'];
        $relationNames = ['accounts', 'sessions', 'userRole'];
    ...
    public function findUnique(array $criteria, string $format = 'array'): array | object
    {
        ...
        if ($format === 'object') {
            return new User($this->_pdo, $record);
        }
    ...
    public function findMany(array $criteria = [], string $format = 'array'): array | object
    {
        ...
        if ($format === 'object') {
            $result = [];
            foreach ($items as $item) {
                $result[] = new User($this->_pdo, $item); // Convert each item into a User object.
            }
            return $result;
        }
                ...
                foreach ($data as $relationName => $relationDataName) {
                    ...
                    if (in_array($relationName, $relationNames)) {
                        ...
                        if (isset($relationDataName['create']) && is_array($relationDataName['create'])) {
                            ...
                            if ($checkArrayContentType !== ArrayType::Value) {
                                throw new \Exception("To create a new record, the value of 'create' must be a single array with names as keys and the data to create as values in  model. example: ['create' => ['name' => 'someName']]");
                            }
                            ...
                            if ($checkArrayContentType !== ArrayType::Associative) {
                                throw new \Exception("To create many records, use an associative array with the field names as keys and the data to create as values in  model. ['createMany' => [['name' => 'someName'], ['name' => 'someOtherName']]");
                            }
                    ...
                    if (isset($relationDataName['connect']) || isset($relationDataName['connectOrCreate']) || isset($relationDataName['disconnect']) || isset($relationDataName['update']) || isset($relationDataName['updateMany'])) {
                        ...
                        if (isset($relationDataName['connect'])) {
                            ...
                            if (isset($relationDataName['connect']) && $checkArrayContentType !== ArrayType::Value) {
                                throw new \Exception("The 'connect' key must be an associative array with the field names as keys and the data to create as values related  model. example: ['connect' => ['id' => 'someId']]");
                            }
                        }
                        ...
                        if (isset($relationDataName['connectOrCreate'])) {
                            ...
                            if (isset($relationDataName['connectOrCreate']) && $checkArrayContentType !== ArrayType::Associative) {
                                throw new \Exception("The 'connectOrCreate' key must be an associative array with the field names as keys and the data to create as values related  model. example: ['connectOrCreate' => ['where' => ['id' => 'someId'], 'create' => ['name' => 'someName']]");
                            }
                        }
                        ...
                        if (isset($relationDataName['disconnect'])) {
                            ...
                            if (!is_bool($disconnectData)) {
                                ...
                                if (isset($relationDataName['disconnect']) && $checkArrayContentType !== ArrayType::Value) {
                                    throw new \Exception("The 'disconnect' key must be an associative array with the field names as keys and the data to create as values related  model. example: ['disconnect' => ['id' => 'someId']]");
                                }
                            }
                        }
    ...
    public function aggregate(array $operation, string $format = 'array'): array | object
    {
        ...
        $quotedTableName = $this->_dbType == 'pgsql' ? "\"User\"" : "`User`";
        $conditions = [];
        $bindings = [];
    ...
    /**
     * Groups records in the 'User' table and performs aggregate operations.
     * 
...
";
    ...
    /**
     * Counts the number of records in the 'User' table based on provided criteria.
     * 
