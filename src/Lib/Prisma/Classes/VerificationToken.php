...
class VerificationToken implements IModel
{
    public $identifier;
    public $token;
    public $expires;
    ...
    public function __construct($pdo, $data = null)
    {
        ...
        $this->_modelName = 'VerificationToken';
        $this->_fieldsOnly = ['identifier', 'token', 'expires'];
        $this->_fieldsRelated = [''];
    ...
    public function setIdentifier($value)
    {
        ...
        $this->identifier = $validatedValue;
    }
    ...
    {
        if (!isset($data['data'])) {
            throw new \Exception("The 'data' key is required when creating a new VerificationToken.");
        }
        ...
        foreach ($requiredFieldsMap as $fieldName => $fieldType) {
            if (!isset($data['data'][$fieldName])) {
                throw new \Exception("The '$fieldName' field of type '$fieldType' is required when creating a new VerificationToken.");
            }
        }
        ...
        $bindings = [];
        $dbType = $this->_dbType;
        $quotedTableName = $dbType == 'pgsql' ? "\"VerificationToken\"" : "`VerificationToken`";
    ...
    public function findUnique(array $criteria, string $format = 'array'): array | object
    {
        ...
        if (isset($where['identifier'])) {
            ...
            $validatedValue = Validator::validateString($where['identifier']);
            $bindings[':identifier'] = $validatedValue;
        }
        if (isset($where['token'])) {
            $conditions[] = "token = :token";
            $validatedValue = Validator::validateString($where['token']);
            $bindings[':token'] = $validatedValue;
        }
        if (empty($conditions)) {
            throw new \Exception("No valid 'where' conditions provided for finding a unique record in VerificationToken.");
        }
        ...
        foreach ($includes as $relation => $include) {
            if ($include) {
                ...
                if (method_exists($this, $includeMethodName)) {
                    ...
                    throw new \Exception("The '$relation' does not exist, in the VerificationToken model.");
                }
            }
        }
        ...
        if ($format === 'object') {
            return new VerificationToken($this->_pdo, $record);
        }
    ...
    public function findMany(array $criteria = [], string $format = 'array'): array | object
    {
        ...
        if ($format === 'object') {
            ...
            foreach ($items as $item) {
                $result[] = new VerificationToken($this->_pdo, $item); // Convert each item into a User object.
            }
            return $result;
        }
    ...
    {
        if (!isset($data['where'])) {
            throw new \Exception("The 'where' key is required in the update VerificationToken.");
        }
        ...
        if (!isset($data['data'])) {
            throw new \Exception("The 'data' key is required in the update VerificationToken.");
        }
        ...
        foreach ($requiredFieldsMap as $fieldName => $fieldType) {
            if (isset($data['data'][$fieldName]) && empty($data['data'][$fieldName])) {
                throw new \Exception("The '$fieldName' field of type '$fieldType' is required in VerificationToken Model.");
            }
        }
                ...
                if (!empty($where)) {
                    ...
                    foreach ($where as $fieldName => $fieldValue) {
                        if (array_key_exists($fieldName, $this->_fields)) {
                            ...
                            throw new \Exception("The '$fieldName' field does not exist in the VerificationToken model.");
                        }
                    }
    ...
    /**
     * @throws \Exception Primary key not defined in model VerificationToken.
     */
    public function upsert($data) 
    {
        throw new \Exception("Primary key not defined in model VerificationToken.");
    }
    ...
    public function aggregate(array $operation, string $format = 'array'): array | object
    {
        ...
        $quotedTableName = $this->_dbType == 'pgsql' ? "\"VerificationToken\"" : "`VerificationToken`";
        $conditions = [];
        $bindings = [];
    ...
    /**
     * Groups records in the 'VerificationToken' table and performs aggregate operations.
     * 
    ...
    {
        if (!isset($data['where'])) {
            throw new \Exception("The 'where' key is required in the updateMany VerificationToken.");
        }
        ...
        if (!isset($data['data'])) {
            throw new \Exception("The 'data' key is required in the updateMany VerificationToken.");
        }
...
";
    ...
    /**
     * Counts the number of records in the 'VerificationToken' table based on provided criteria.
     * 
     ...
     * $activeUserCount = $prisma->VerificationToken->count($countCriteria);
     */
